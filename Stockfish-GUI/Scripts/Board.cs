...
namespace Stockfish_GUI
{
    public class Board
    {
        ...
        public void MoveTo(Point from, Point to, bool animate)
        {
            ...
            int length = Movements.Count - MovementFocus;
        ...
        public void Move(Movement movement, bool animate)
        {
            ...
            Form.TimeBar.Value = MovementFocus;
        ...
        public void Redo(bool animate = true)
        {
            if (MovementFocus > Movements.Count - 1)
                return;

            Move(Movements[MovementFocus], animate: true);
        }

        public void Undo(bool animate = true)
        {
            if (MovementFocus <= 0)
                return;

            Undo(Movements[MovementFocus - 1], animate: true);
        }
        ...
        public struct Movement
        {
            ...
            public List<Point> Footprints = new();

            public Movement(Point from, Point to, Piece piece, Piece Captured)
            {
                From = from;
                To = to;
            }
